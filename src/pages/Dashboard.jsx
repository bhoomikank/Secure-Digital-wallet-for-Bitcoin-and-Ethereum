...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadWallets() {
      try {
        ...
        const res = await fetch("http://localhost:5000/api/wallet/eth", {
  ...
  useEffect(() => {
    ...
    fetchPrice();
    const interval = setInterval(fetchPrice, 15000); // refresh every 15 sec
    ...
    <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-6 bg-black text-white">
      ...
      <div className="p-6 rounded-lg  w-full text-center shadow-lg flex flex-col items-center justify-center">
        <div className="w-full sm:w-[70%] md:w-[50%] lg:w-[40%] h-auto p-6 shadow-lg flex flex-col items-center justify-center rounded-2xl bg-orange-500 hover:bg-gray-400 transition-all">
          ...
          {/* USD + INR Price */}
          {ethPriceUSD && balanceEth && (
            <div className="text-sm mt-2 text-black font-bold bg-yellow-300 p-2 rounded-lg">
              <p>USD: ${(balanceEth * ethPriceUSD).toFixed(2)}</p>
              <p>INR: ₹{(balanceEth * ethPriceINR).toFixed(2)}</p>
            </div>
          )}
        ...
        <div className="mt-8 border-t border-gray-700 pt-6">
          ...
          {/* Live conversion while sending */}
          {ethPriceUSD && amount && (
            <div className="text-sm mb-3 text-black font-bold bg-yellow-300 p-2 rounded-lg">
              <p>USD: ${(amount * ethPriceUSD).toFixed(2)}</p>
              <p>INR: ₹{(amount * ethPriceINR).toFixed(2)}</p>
            </div>
          )}
